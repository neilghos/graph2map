...
def compute_graph_spectrograms(
    ...
    c0, c1, c2 = min_max_norm(c0), min_max_norm(c1), min_max_norm(c2)
...
def extract_node_4ch_spatial_fast(
    ...
    """
    Ultra-fast vectorized extraction of 4 spatial cartography channels (<1.5ms per node):
      - Ch 0: Ego Density Field
      - Ch 1: Subgraph Edge Line Flux
      - Ch 2: Spatial PPR Heat
      - Ch 3: Relative Feature Homophily
    """
    ...
    # 1. Concentric Layout
    coords, hop_dist = compute_ego_layout_fast(num_sub_nodes, sub_edge_index, target_idx, max_hop=num_hops)

    # 2. Vectorized Node Distance Field: (H, W, N)
    diff_nodes = grid.unsqueeze(2) - coords.view(1, 1, num_sub_nodes, 2)
    dist_sq_nodes = (diff_nodes ** 2).sum(dim=-1)
    kernel_nodes = torch.exp(-dist_sq_nodes / (2.0 * sigma_node ** 2))

    # Ch 0: Ego Density Field
    hop_weights = torch.tensor([1.0 / max(d, 0.5) if d > 0 else 2.5 for d in hop_dist], dtype=torch.float32)
    ...
    if num_sub_edges > 0:
        p0 = coords[sub_edge_index[0]]
        p1 = coords[sub_edge_index[1]]
        v = p1 - p0
        ...
        ego_edges = torch.zeros((resolution, resolution), dtype=torch.float32)

    # Ch 2: Fast Spatial PPR Heat Return
    ppr_weights = compute_spatial_ppr_fast(num_sub_nodes, sub_edge_index, target_idx)
    ...
    if sub_x is not None:
        ...
        cos_sim = F.cosine_similarity(sub_x.float(), target_f, dim=-1)
        homophily_weights = (cos_sim + 1.0) * 0.5
    else:
        homophily_weights = torch.ones((num_sub_nodes,), dtype=torch.float32)
    ...
    return torch.stack(norm_chans, dim=0)
...
def save_atlas_panel(
    ...
    draw.text((20, 38), "Row 1: 3 Spectral Channels (Bicubic) | Row 2: 4 Spatial Cartography Fields (ego_maps_output)", fill=(160, 175, 195))
...
def get_or_create_atlas_cache(dataset, edge_index_cpu, x_cpu, args) -> torch.Tensor:
    ...
    cache_file = os.path.join(
        cache_dir,
        f"{args.dataset}_atlas7ch_res{resolution}_hops{num_hops}_bands{effective_bands}.pt"
    )

    if os.path.exists(cache_file):
        print(f"Loading pre-computed 7-Channel Visual Atlas cache from: {cache_file} ...")
        t0 = time.time()
        cached_atlas = torch.load(cache_file)
        print(f"Loaded {cached_atlas.shape[0]} Visual Atlases in {time.time() - t0:.2f}s | Tensor: {list(cached_atlas.shape)} ({cached_atlas.dtype})")
        return cached_atlas

    print(f"\n{'='*75}")
    print(f"Generating 7-Channel Master Visual Atlas (7 x {resolution}x{resolution}) for '{args.dataset}' ({n} nodes)")
    print(f"Engine: Ultra-Fast Pure PyTorch Vectorization (<30s total runtime)")
    ...
    # Step 1: Compute 3-channel spectrograms globally (<0.3s)
    print("Step 1/2: Computing global 3-channel spectrograms...")
    ...
    # Batched bicubic resize of all 7650 spectrograms simultaneously (<0.15s)
    print("-> Batched bicubic resizing of all spectrogram channels to 128x128...")
    ...
    # Step 2: Vectorized spatial rasterization
    print("Step 2/2: Vectorized spatial cartography extraction...")
    cached_atlas = torch.zeros((n, 7, resolution, resolution), dtype=torch.uint8)
    ...
    pbar = tqdm(range(n), desc=f"Building 7ch Atlas [{args.dataset}]", unit="node", dynamic_ncols=True)
    for i in pbar:
        # 3 Spectral Channels from batched resize
        spec_3ch_uint8 = (all_specs_resized[i] * 255.0).clamp(0, 255).to(torch.uint8)

        # 4 Spatial Channels computed in ~1.5ms
        ...
        cached_atlas[i, :3] = spec_3ch_uint8
        cached_atlas[i, 3:] = spatial_4ch_uint8

    torch.save(cached_atlas, cache_file)
    size_mb = (cached_atlas.element_size() * cached_atlas.nelement()) / (1024 * 1024)
    elapsed_total = time.time() - t0
    rate = n / max(elapsed_total, 0.001)
    print(f"\nAtlas caching complete in {elapsed_total:.2f}s ({rate:.1f} nodes/s)! Saved {size_mb:.1f} MB to: {cache_file}\n")
    ...
    try:
        ...
        sample_tensor = (cached_atlas[0].float()) / 255.0
...
if __name__ == '__main__':
    ...
    all_specs, _ = compute_graph_spectrograms(edge_index, x, num_nodes, num_hops=8, num_bands=128)
    all_specs_resized = F.interpolate(all_specs[:100].float(), size=(128, 128), mode='bicubic', align_corners=False)

    lin = torch.linspace(-1.0, 1.0, 128)
    grid_y, grid_x = torch.meshgrid(lin, lin, indexing='ij')
    grid = torch.stack([grid_x, grid_y], dim=-1)

    t0 = time.time()
    for i in range(100):
        _ = extract_node_4ch_spatial_fast(i, edge_index, x, grid=grid, num_hops=2, resolution=128)
    dt = time.time() - t0
    rate = 100 / dt
    ...
    # Save Node 0 sample
    sample_spatial = extract_node_4ch_spatial_fast(0, edge_index, x, grid=grid, num_hops=2, resolution=128)
    sample_atlas = torch.cat([all_specs_resized[0], sample_spatial], dim=0)
